...
export default function UserEditPage() {
  ...
  function validate() {
    ...
    if (form.password && form.password.length < 6) errs.password = '密碼至少 6 個字元'
  ...
  if (loading) return <div className="user-loading">載入中...</div>
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="form-group">
          ...
          <input id="password" name="password" type="password" className={`form-input${errors.password ? ' error' : ''}`}
            value={form.password} onChange={handleChange} placeholder="留空則不變更密碼" disabled={submitting} />
          {errors.password && <div className="form-error">{errors.password}</div>}
          <div className="form-hint">留空則不變更密碼</div>
        </div>
