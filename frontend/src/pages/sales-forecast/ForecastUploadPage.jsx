...
export default function ForecastUploadPage() {
  ...
  const runQuery = useCallback(async (optVersion) => {
    ...
    try {
      ...
      const versionToUse = optVersion ?? (canUpdateAfterClosed && versionStrings.length > 0 ? versionStrings[0] : '')
  ...
  }, [selectedMonth, selectedChannel, canUpdateAfterClosed, toast])
        ...
        <>
          ...
          <section className="upload-block upload-block--query">
            ...
            <div className="upload-block-filters">
              ...
              {canUpdateAfterClosed && (
                <div className="filter-field filter-field--inline">
                  <label htmlFor="query-version">版次</label>
                  <select
                    id="query-version"
                    className="form-select form-select--sm"
                    value={selectedVersion}
                    onChange={(e) => setSelectedVersion(e.target.value)}
                    disabled={!queryClicked || versions.length === 0}
                  >
                    <option value="">請先查詢</option>
                    {versions.map((v) => (
          ...
          <section className="upload-block upload-block--upload">
            ...
            <div className="upload-form">
              ...
              <div className="upload-form-actions">
                ...
                {!canUploadSection && canUpdateAfterClosed && selectedMonth && selectedChannel && (
                  editingNewVersion ? (
                    <>
                      <button
                        type="button"
                        className="btn btn--primary"
                        onClick={handleSaveVersionWithReason}
                      >
                        儲存
                      </button>
                      <button
                          ...
                          <table className="forecast-table">
                            ...
                            <tbody>
                              {pageData.map((item) => {
                                ...
                                const isQuantityModified = Boolean(item.is_modified ?? item.isModified)
                                return (
                                  <tr key={item.id} className={isNewRow ? 'row-added' : ''}>
                                    ...
                                    <td className={`align-right${isQuantityModified ? ' quantity-cell--modified' : ''}`}>
                                      {isEditing ? (
                                        <input
                                          ref={editInputRef}
                                          type="number"
